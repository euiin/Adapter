...
from LoRASYM_peft.local_peft_model_all import PeftModel_local, PeftModelForSequenceClassification_local,\
                                            ...
                                            set_peft_model_safetensors_local
...
def strategy_to_rule_dict(update_A=True, update_B=True, update_E=True,
        ...
        print("strategy_to_rule_dict =", para_dict)
        return para_dict
...
def STRATEGY_TO_RULE_func(strategy_str):
    ...
    return strategy_to_rule_dict(update_E=False, use_E=False,
                                ...
                                A_init=command_list[1], B_init=command_list[3])

def load_peft_model_pretrained(model, method, strategy, model_config, lora_path, task_type_input="SEQ_CLS"):
    ...
    if method == "LoRASYM":
        ...
        # 근데 update_rule에 맞는 layer만 불러오는 함수는 필요할 수도 있음.
        lora_model = TASKTYPE_TO_PEFTMODEL[task_type_input].from_pretrained(model, lora_path)
        # set_peft_model_safetensors_local(lora_model, lora_path+'/adapter_model.safetensors', adapter_name = "default")
        
        if update_rule_dict["update_B"] == True:
        ...
        lora_config = LoraConfig(
                        r=model_config.lora_rank,    #  # default is 8
...
def load_peft_model_test(model, method, strategy, model_config, task_type_input="SEQ_CLS"):
...
def load_peft_model_test_2(model, method, strategy, model_config, task_type_input="SEQ_CLS"):
