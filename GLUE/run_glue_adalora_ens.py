...
from loralib.adalora import RankAllocator, compute_orth_regu 
...
class ModelArguments:
    ...
    adalora_path: Optional[str] = field(
        default=None,
        metadata={"help": "The file path of LoRA parameters."},
    )
...
def main():
    ...
    if model_args.apply_adalora:
        ...
        config = AdaLoraConfig(
            ...
            lora_alpha=model_args.lora_alpha, 
            target_modules=["intermediate.dense"],#["intermediate.dense", "output.dense"], 
            bias="none",
    ...
    if model_args.adalora_path is not None and not os.path.exists(model_args.adalora_path + "/adapter_model.safetensors"):
        tensors = {}
        with safe_open(model_args.adalora_path + "/model.safetensors", framework="pt", device=0) as f:
            for k in f.keys():
                n_k =  'base_model.model.' + k
                tensors[n_k] = f.get_tensor(k) # loads the full tensor given a key
        new_model = model_args.adalora_path + "/adapter_model.safetensors"
        save_file(tensors, new_model)
    ...
    if model_args.apply_adalora:
        if model_args.adalora_path is not None and training_args.do_eval:
            print("load adalora model")
            # config = PeftConfig.from_pretrained(model_args.adalora_path)
            model = PeftModel.from_pretrained(model, model_args.adalora_path)
            model.print_trainable_parameters()
            model = model.model
        else:
            model = get_peft_model(model, config)
            model.print_trainable_parameters()
            # model = model.model # Unwrapping the PeftModel
            # model.classifier.original_module.out_proj.out_features=3
            # model.classifier.modules_to_save.default.out_proj.out_features=3
    ...
    elif model_args.lora_type == "svd": # model_args.apply_adalora:
        trainer = Trainer(
            model=model,
            args=training_args,
            train_dataset=train_dataset if training_args.do_train else None,
