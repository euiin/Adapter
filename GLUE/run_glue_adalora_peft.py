...
def save_adapter_model(model, output_dir, filename="adapter_model.safetensors"):
    ...
    # Save the model's state dictionary using safetensors
...
def main():
    ...
    if model_args.apply_lora:
        config = AutoConfig.from_pretrained(
            ...
            #dropout=training_args.cls_dropout,
            apply_lora=model_args.apply_lora,
            # 2 below args is for adalora
            lora_type=model_args.lora_type, 
            lora_module=model_args.target_modules, 
    ...
    elif model_args.apply_adalora:
        print("AdaLoRA init")
        config = AdaLoraConfig(
            target_r=model_args.target_rank,
            init_r=model_args.lora_r,
            lora_alpha=model_args.lora_alpha, 
            target_modules=model_args.target_modules, 
            bias="none",
            modules_to_save=model_args.modules_to_save, 
            total_step=training_args.num_train_epochs * len(datasets["train"]),
            tinit=model_args.init_warmup,
            tfinal=model_args.final_warmup,
            beta1=model_args.beta1,
            beta2=model_args.beta2,
            orth_reg_weight=model_args.reg_orth_coef,
        )
    ...
    if training_args.do_train:
        ...
        train_dataloader = DataLoader(
            train_dataset, shuffle=True, collate_fn=default_data_collator, batch_size=training_args.per_device_train_batch_size, pin_memory=True
        )
    ...
    if training_args.do_eval:
        ...
        eval_dataloader = DataLoader(eval_dataset, collate_fn=default_data_collator, batch_size=training_args.per_device_train_batch_size, pin_memory=True)
    ...
    if training_args.do_predict or data_args.task_name is not None or data_args.test_file is not None:
        ...
        test_dataloader = DataLoader(test_dataset, collate_fn=default_data_collator, batch_size=training_args.per_device_train_batch_size, pin_memory=True)
    ...
    ###############################
    expected_keys = {"input_ids", "attention_mask", "token_type_ids", "labels"}
    ...
    for epoch in range(int(training_args.num_train_epochs)):
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            batch = {k: v.to('cuda') for k, v in batch.items() if k in expected_keys}
            outputs = model(**batch)
            loss = outputs.loss
        ...
        for step, batch in enumerate(eval_dataloader):
            #batch = {k: v.to('cuda') for k, v in batch.items() if k in expected_keys}
            batch = {k: v.to(accelerator.device) for k, v in batch.items() if k in expected_keys}
            with torch.no_grad():
                # del batch["idx"]
                outputs = model(**batch)
        ...
        eval_epoch_loss =  eval_metric / len(train_dataloader)
        eval_ppl = torch.exp(eval_epoch_loss)
        train_epoch_loss = total_loss / len(eval_dataloader)
        train_ppl = torch.exp(train_epoch_loss)
        ...
        wandb.log({
            ...
            "Eval PPL": eval_ppl.item(),
            "Train Loss": train_epoch_loss.item(),
    ...
    if training_args.do_predict:
        ...
        # Log test metrics
        wandb.log({"test_preds": test_preds})
        
        # print accuracy
        correct = 0
        total = 0
        for pred, true in zip(test_preds, dataset["test"]["text_label"]):
            if pred.strip() == true.strip():
                correct += 1
            total += 1
        accuracy = correct / total * 100
